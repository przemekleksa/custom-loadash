...
dropWhile([1, 2, 3, 4, 5, 6], function(o) { return o <= 3; })
...
const find = (array, func, index = 0) => {
	...
	return parseInt(foundArr.toString())
}
...
const zip = (...array) => {
	...
	let zippedArr = []
	for (i = 0; i < argsArr[0][0].length; i++) {
		zippedArr[i] = []
	}
	for (let i = 0; i < argsArr.length; i++) {
		for (let j = 0; j < argsArr[i].length; j++) {
			for (let k = 0; k < argsArr[i][j].length; k++) {
				zippedArr[k][zippedArr[0].length] = argsArr[i][j][k]
			}
		}
	}
	return zippedArr
...
zip(['a', 'b', 'c'], [1, 2, 3], [true, false, 'else'])
...
const pickBy = (object, func) => {
	...
	for (i in object) {
		...
		if (isValueOK === true) {  
			setProp(finalObject, i, object[i])
		}
	}
	return finalObject
}
...
pickBy(objectToPick, function(o){return typeof a === 'number'})
...
const toPairs = (object) => {
    let arrayOfPairs = []
	for (let [key, value] of Object.entries(object)) {
		arrayOfPairs[arrayOfPairs.length] = [key, parseInt(value)]
	}
    return arrayOfPairs
  }
...
module.exports = { chunk, compact, drop, dropWhile, take, filter, find, includes, map, zip, setProp, merge, omit, omitBy, pick, pickBy, toPairs }